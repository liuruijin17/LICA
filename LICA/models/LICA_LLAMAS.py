...
from .py_utils import kp, AELoss, SelfAttentionConv
...
class SelfBottleneck(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None,
                 kernel_size=None, padding=None, attn_groups=None, embed_shape=None):
        super(SelfBottleneck, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        if groups != 1 or base_width != 64:
            raise ValueError('BasicBlock only supports groups=1 and base_width=64')
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in BasicBlock")
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.conv1 = SelfAttentionConv(inplanes, planes, kernel_size=kernel_size, padding=padding, groups=attn_groups, stride=stride)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = SelfAttentionConv(planes, planes, kernel_size=kernel_size, padding=padding, groups=attn_groups)
        self.bn2 = norm_layer(planes)
        self.downsample = downsample
        self.stride = stride
...
class model(kp):
    def __init__(self, db, flag=False, freeze=False):
        ...
        if system_configs.block == 'BasicBlock':
            ...
            block = [BasicBlock, BasicBlock, SelfBottleneck, SelfBottleneck]
